...
The distributed password recovery framework was deployed in a virtualized enviroment. The enviroment was provided by the Centre for Research on Cryptography and Security (CRoCS). The setup consisted of 4 machines. One machine was used as the control server, the remaining three were clients. All were part of one local subnet. Every machine had 4 virtualized CPUs and 2 GB of RAM available. 
...
Note that the PDF variants were choosen to highlight differences between multiple protection mechanism. These algorithms changed frequently in obscure ways, as discussed in section \ref{pdf_summ}.

Every test had a duration of six hours. The password chunk size was set to 50,000, e.g. every client received a list of 50,000 unique passwords after connecting to the server. 
...
Not so suprisingly, see section \ref{pdf_summ}, the highest password throughput was achieved when testing a PDF document using a security handler of version 4 revision 5. Adobe's obscure algorithm used in handlers of revision 6 is better. However, basically it is just as effective as 1024 iterations of PBKDF2 (used by LibreOffice Writer 5.2.2).  Worth mentioning is also the negligible difference between PDF's security handler revision 2 and 4. Adding tens of iterations of RC4 encryption has as good as no security benefits. 
...
It should be mentioned that these results do no represent the brute-force capabilities of modern technology. Any decent tool leveraging parallelism provided by high-end GPUs will most probably achieve better results in terms of performance. Further work may include implementing such tools as clients for the distributed password recovery system. 
