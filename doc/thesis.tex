...
The aim of this thesis is to provide an overview of protection mechanisms implemented by three popular and widely used formats -- Microsoft Office documents, Portable Document Format (PDF) files, and Open Document Format (ODF) documents. To achieve this goal an rather in depth knowledge of this document formats is needed. Fortunately, all mentioned format provide publicly available documentation. Therefore, in the beginning a research covering all formats was done. 
...
\subsection{Password verification process}
...
As JtR was not extensively used when working on this thesis it is mainly mentioned for the sake of completeness.

John The Ripper (JtR), similarly to hashcat, is an open-source password recovery utility. It is supported on multiple operating system including many flavors of Unix, Windows, DOS, and OpenVMS. A paid version of JtR is also available. The official website states that this version is meant to be easier to install and use delivering optimal performance \cite{jtr}. Worth mentioning is also the community-enhanced version labeled \textit{jumbo}. It includes a lot of code, documentation, and data contributed by the user community around JtR.
