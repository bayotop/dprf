...
When agile encryption is used all information about the cryptography used to encrypt the document is stored in an XML element in the \textit{XmlEncryptionDescriptor} field of the \textit{EncryptionInfo} stream. This element must conform the XML schema namespace defined in \ref{ei_agilestream}. Agile encryption allows a wide-range of possible configurations and therefore is much more complex than standard encryption discussed in the previous section. The available algorithms depend on the algorithms that can be accessed through the APIs in the Windows operating system. Microsoft Office 2007 with Service Pack 2 and above support CNG (CryptoAPI: Next Generation). The following listing shows the settings and their default values, which are configurable and change the way how files are being encrypted:
...
\item{Calculate a final hash:}
	...
	\end{itemize}
\item{If the length of $H_{final}$ is smaller than the key length specified in the \textit{EncryptionInfo}, the key must be padded by appending bytes with a value of $0x36$. If it is longer, it is truncated to the correct length.}
\end{enumerate}
...
\begin{enumerate}
	...
	\end{itemize}
	\item{If a \textit{blockKey} is not provided the IV is the \textit{KeySalt}.}
...
To understand how the password key encryptor is generated, it's necessary to describe the meaning of some of its elements:
...
In addition to confidentiality, agile encryption also provides structures that are used to verify integrity of encrypted files. These structures are stored in the \textit{EncryptionInfo's DataIntegrity} element. The process of generating this element is further described in \ref{data_integrity}. 
